...
const Game = class {
	constructor(termStrArray) {
		const terms = termStrArray.map(Game.parseTermString);
		D(this, {
			terms
		}, false);

		const numOfTerms = terms.length,
			numOfValues = terms[0].values.length,
			numOfRows = numOfTerms * numOfValues,
			numOfCells = S.natSum(numOfTerms - 1) * (numOfValues * numOfValues);
		D(this, {
			numOfTerms,
			numOfValues,
			numOfRows,
