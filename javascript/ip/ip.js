...
const arrayConcat = arrayOfArraies => Array.prototype.concat.apply([], arrayOfArraies);

// 将任意数值 value 转成 min 到 max 之间的整数。
// 非数值返回 0；有小数的截掉小数部分仅取整数；大于最大值的取最大值；小于最小值的取最小值。
const intClip = (min, max) => value => {
...
const toInt53 = intClip(Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
...
const parseIpString = ipStr => {
	if (/^(\d|([1-9]\d)|(1\d\d)|(2[0-4]\d)|(25[0-5]))(\.(\d|([1-9]\d)|(1\d\d)|(2[0-4]\d)|(25[0-5]))){3}$/.test(ipStr)) {
...
const toIpString = ip => [(ip >>> 24) & 0xff, (ip >>> 16) & 0xff, (ip >>> 8) & 0xff, (ip >>> 0) & 0xff].join(".");
...
const cidrFromValidNetmask = netmask => 32 - intLog2(((netmask ^ 0xffffffff) >>> 0) + 1);
...
// 给出子网对应的起止 IP。
const segmentFromSubnet = ([ip, netmask]) => segmentFromCidr(ip, cidrFromNetmask(netmask));
...
const subnetsFromSegments = segments => arrayConcat(segments.map(subnetsFromSegment));
...
const mergeAdjacentSegments = segments => {
	...
	segments.sort((s1, s2) => (s1.ipBegin - s2.ipBegin)).forEach(
		segment => {
			const {
				ipBegin,
				ipEnd
			...
			} else if (ipBegin >= saveSegment.ipBegin && ipBegin <= saveSegment.ipEnd + 1) {
...
const mergeAdjacentSubnets = subnets => arrayConcat(mergeAdjacentSegments(subnets.map(segmentFromSubnet)).map(subnetsFromSegment));
...
const toSegmentString = subnets => {
	return mergeAdjacentSegments(subnets.map(segmentFromSubnet)).map(
		s => {
...
const parseRangeString = (rangeStr, mergeAdjacent = true) => {
	const segments = rangeStr.split(/\s*[,;]\s*/).map(
		s => {
	...
	return subnetsFromSegments(mergeAdjacent ? mergeAdjacentSegments(segments) : segments);
};
